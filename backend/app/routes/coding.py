...
class CoreSubjectUpdateSchema(BaseModel):
    ...
    checked_indices: Optional[List[int]] = None
...
async def get_progress(user: User = Depends(get_current_user)):
    ...
    if progress.active_core_subjects_checked is None:
    ...
    if progress.active_aptitude_checked is None:
    ...
    for subject in ["DBMS", "OS", "CN", "OOP"]:
        if subject not in progress.core_subjects_total_generated:
            ...
            progress.active_core_subjects_checked[subject] = []
            need_save = True
    ...
    for topic in ["Quantitative Aptitude", "Logical Reasoning", "Verbal Ability"]:
        if topic not in progress.aptitude_total_generated:
            ...
            progress.active_aptitude_checked[topic] = []
            need_save = True
...
async def update_core_subject(data: CoreSubjectUpdateSchema, user: User = Depends(get_current_user)):
    ...
    if data.subject not in progress.core_subjects_total_generated:
        ...
        progress.active_core_subjects_checked[data.subject] = []

    # Calculate delta in active solved count
    if data.checked_indices is not None:
        new_active_solved = len(data.checked_indices)
        old_active_solved = len(progress.active_core_subjects_checked.get(data.subject, []))
        progress.active_core_subjects_checked[data.subject] = data.checked_indices
        progress.active_core_subjects_checked = progress.active_core_subjects_checked.copy()
        progress.active_core_subjects_progress[data.subject] = new_active_solved * 20.0
    else:
    ...
    progress.active_core_subjects_progress = progress.active_core_subjects_progress.copy()
    ...
    progress.core_subjects_total_solved = progress.core_subjects_total_solved.copy()
    ...
    progress.core_subjects_progress = progress.core_subjects_progress.copy()
...
async def update_aptitude(data: AptitudeUpdateSchema, user: User = Depends(get_current_user)):
    ...
    if data.topic not in progress.aptitude_total_generated:
        ...
        progress.active_aptitude_checked[data.topic] = []

    # Calculate delta in active solved count
    if data.checked_indices is not None:
        new_active_solved = len(data.checked_indices)
        old_active_solved = len(progress.active_aptitude_checked.get(data.topic, []))
        progress.active_aptitude_checked[data.topic] = data.checked_indices
        progress.active_aptitude_checked = progress.active_aptitude_checked.copy()
        progress.active_aptitude_progress[data.topic] = new_active_solved * 20.0
    else:
        new_active_solved = round(data.completion_percentage / 20.0)
    ...
    progress.active_aptitude_progress = progress.active_aptitude_progress.copy()
    delta = new_active_solved - old_active_solved
    ...
    progress.aptitude_total_solved = progress.aptitude_total_solved.copy()
    ...
    progress.aptitude_progress = progress.aptitude_progress.copy()
...
async def regenerate_core_subjects_questions(data: RegenerateQuestionsSchema, user: User = Depends(verify_ai_rate_limit)):
    ...
    # Reset active progress to 0% and clear checked checkboxes for the new batch
    ...
    # Make dictionary copies to guarantee Beanie saves changes
    progress.core_subjects_questions = progress.core_subjects_questions.copy()
    progress.core_subjects_total_generated = progress.core_subjects_total_generated.copy()
    progress.core_subjects_total_solved = progress.core_subjects_total_solved.copy()
    progress.active_core_subjects_progress = progress.active_core_subjects_progress.copy()
    progress.active_core_subjects_checked = progress.active_core_subjects_checked.copy()
    progress.core_subjects_progress = progress.core_subjects_progress.copy()
...
async def regenerate_aptitude_questions(data: RegenerateAptitudeQuestionsSchema, user: User = Depends(verify_ai_rate_limit)):
    ...
    # Make dictionary copies to guarantee Beanie saves changes
    progress.aptitude_questions = progress.aptitude_questions.copy()
    progress.aptitude_total_generated = progress.aptitude_total_generated.copy()
    progress.aptitude_total_solved = progress.aptitude_total_solved.copy()
    progress.active_aptitude_progress = progress.active_aptitude_progress.copy()
    progress.active_aptitude_checked = progress.active_aptitude_checked.copy()
    progress.aptitude_progress = progress.aptitude_progress.copy()
